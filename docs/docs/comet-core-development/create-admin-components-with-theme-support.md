...
```tsx
import { RootSlotProps } from "@comet/admin";
...
/**
 * The `{ComponentName}Props` type defines all of the component's props and should extend `RootSlotProps`.
 * `RootSlotProps` adds the props necessary for overriding the component's styles by adding the `sx`, `className` and `slotProps` props.
 * If the component has multiple slots, pass in a generic/object with the slots as keys and their types as values.
 */
export interface MyComponentProps
    extends RootSlotProps<{
        root: "div";
        header: "div";
